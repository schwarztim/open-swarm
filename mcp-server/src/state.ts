...
export function buildManagerPrompt(
  ...
  return [
    ...
    "When you dispatch workers, include these in EVERY worker prompt:",
    "```",
    `SESSION_ID: ${session.id}`,
    `GROUP_ID: ${group.id}`,
    "WORKSTREAM_ID: <worker's workstream id>",
    "```",
    "",
    "Tell each worker:",
    ...
    `   swarm_relay(sessionId="${session.id}", workstream="<ws-id>", level="L3", group="${group.id}",`,
    '     type="finding", content="<what you found>")',
    "3. **If blocked**: Post a blocker — do NOT guess:",
    `   swarm_relay(sessionId="${session.id}", workstream="<ws-id>", level="L3", group="${group.id}",`,
    '     type="blocker", content="<question or issue>")',
    "   Then continue with your best judgment, noting the assumption.",
