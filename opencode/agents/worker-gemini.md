---
description: "Google worker for swarm workstreams. Routes here when provider=google."
mode: subagent
...
1. Use a DIFFERENT provider than yourself for diversity: prefer `worker-anthropic`, `worker-openai`, or `worker-haiku`
...
```
task(subagent_type="worker-anthropic", description="Implement core logic", prompt="...")
task(subagent_type="worker-openai", description="Write test coverage", prompt="...")
```
